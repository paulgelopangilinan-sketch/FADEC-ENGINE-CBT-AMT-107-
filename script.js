...
function renderLiveEventLog(targetId) {
    const log = $(targetId);
    if (!log) return;

    if (!liveEventHistory.length) {
        log.textContent = "WAITING FOR SYSTEM ACTIVITY";
        return;
    }
...
function pushLiveEvent(message, kind = "system") {
    ...
    const mcduLog = $("faultLog");
    if (!log && !mcduLog) return;
    ...
    // The MCDU EVENT LOG mirrors the Controls EVENT LOG exactly.
    renderLiveEventLog("liveEventLog");
    renderLiveEventLog("faultLog");

    // Keep the user's current position. Only follow the feed when they were
    // already viewing the newest entries at the bottom.
    if (log && wasNearBottom) {
        log.scrollTop = log.scrollHeight;
    }

    if (mcduLog) {
        mcduLog.scrollTop = mcduLog.scrollHeight;
    }
}
...
function updateFaultLog() {
    ...
    if (
        ...
        // The MCDU EVENT LOG is a live mirror, not a fault-only list.
        renderLiveEventLog("faultLog");
    ...
    // Keep the MCDU EVENT LOG synchronized with the live Controls EVENT LOG.
    renderLiveEventLog("faultLog");
...
/* =========================================================
   PHONE LANDSCAPE AUTO-FIT
   UI-only helper. Does not touch simulator, FADEC, or audio logic.
   ========================================================= */
(function setupPhoneLandscapeUI(){
    function applyPhoneViewport(){
        var isSmallLandscape = window.matchMedia && window.matchMedia('(max-width: 900px) and (orientation: landscape)').matches;
        if (isSmallLandscape) {
            document.body.classList.add('phone-ui');
            var b = document.getElementById('phoneUiBtn');
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function(){
            applyPhoneViewport();
            window.addEventListener('resize', applyPhoneViewport, {passive:true});
            window.addEventListener('orientationchange', function(){ setTimeout(applyPhoneViewport,100); }, {passive:true});
        }, {once:true});
    } else {
        applyPhoneViewport();
        window.addEventListener('resize', applyPhoneViewport, {passive:true});
